...
import time
import sys
sys.path.append("/home/matthias/Dokumente/Uni/auto-sklearn")
sys.path.append("/home/matthias/Dokumente/Uni/Auto-PyTorch/submodules/HpBandSter")
...
from autosklearn.metrics import roc_auc, balanced_accuracy, f1_macro, \
    pac_score, r2
from autosklearn.util.backend import create

metric_name_to_function = {'auc_metric': roc_auc,
                           'bac_metric': balanced_accuracy,
                           'f1_metric': f1_macro,
                           'pac_metric': pac_score,
                           'r2_metric': r2,
                           'roc_auc': roc_auc}


def read_autopytorch_ensemble_prediction_file(filename):
...
def convert_files(auto_pytorch_files, starttime_diff, mean):
	all_predictions, all_timestamps = auto_pytorch_files
	all_timestamps = [x["finished"] + starttime_diff for x in all_timestamps]
	return [
		(np.mean(all_predictions[i], axis=0) if mean else all_predictions[i], ("autopytorch_key", i), all_timestamps[i])
...
def main(input_directories, input_directory_autopytorch, output_file, seed, ensemble_size, n_jobs=1):
    ...
    autopytorch_result = logged_results_to_HBS_result(input_directory_autopytorch)
    autopytorch_starttime = autopytorch_result.HB_config["time_ref"]
    starttime_diff = starttime - autopytorch_starttime
    validation_files.extend(convert_files(
    	read_autopytorch_ensemble_prediction_file(
    		os.path.join(input_directory_autopytorch, "predictions_for_ensemble.npy")
    	),
    	starttime_diff,
    	False
    ))
    test_files.extend(convert_files(
    	read_autopytorch_ensemble_prediction_file(
    		os.path.join(input_directory_autopytorch, "test_predictions_for_ensemble.npy")
    	),
    	starttime_diff,
    	True
    ))
    ...
    dataset_name = D.name
    if dataset_name == "alexis":
        score = D.info['metric']
    elif dataset_name == "yolanda":
        score = D.info['metric']
    elif dataset_name == "tania":
        score = D.info['metric']
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input-directory-autopytorch', type=str, required=True)
    ...
    input_directory_autopytorch = args.input_directory_autopytorch
    output_file = args.output_file
    seed = args.seed
    ensemble_size = args.ensemble_size
    n_jobs = args.n_jobs

    main(input_directory, input_directory_autopytorch, output_file, seed, ensemble_size, n_jobs)
